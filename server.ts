...
if (!defaultShop) {
  db.prepare(
    "INSERT INTO shops (id, slug, name, closingTime, status, password, bankInfo) VALUES (?, ?, ?, ?, ?, ?, ?)"
  ).run(
    defaultShopId,
    "quan-cua-tui",
    "Quán Của Tui",
  ...
  const insertMenu = db.prepare(
    "INSERT INTO menu (id, shopId, name, price, isAvailable, imageUrl) VALUES (?, ?, ?, ?, ?, ?)"
  );
...
// Helpers
// ================================
const getShopId = (req: any) => String(req.query.shopId || defaultShopId);
...
app.post("/api/auth", (req, res) => {
  ...
  if (password && password === shop.password) res.json({ token: password });
...
app.post("/api/orders", (req, res) => {
  ...
  const insertItem = db.prepare(
    "INSERT INTO order_items (orderId, shopId, menuItemId, quantity, price, name) VALUES (?, ?, ?, ?, ?, ?)"
  );
